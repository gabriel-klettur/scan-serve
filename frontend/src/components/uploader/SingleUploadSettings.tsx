...
import { Cpu, Sparkles, Zap, Clock, Users, Mail, Bell, BellOff } from 'lucide-react';
...
interface SingleUploadSettingsProps {
  disabled?: boolean;
}

export const SingleUploadSettings = ({ disabled = false }: SingleUploadSettingsProps) => {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          <div
            className={cn(
              ...
              applyAiEnhancement ? 'bg-primary/20 text-primary' : 'bg-secondary text-muted-foreground',
            )}
          >
      ...
      <div className="flex items-center gap-2 p-3 rounded-lg bg-warning/10 border border-warning/20">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {supportsParallel && selectedEngine === 'vision' && (
              <>
                {' • '}
                <span className="text-foreground font-medium">parallel supported</span>
              </>
            )}
          </p>
        </div>
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger
          className={cn(
            'flex items-center justify-between w-full p-2 rounded-lg hover:bg-secondary/50 transition-colors',
            disabled && 'opacity-60 pointer-events-none',
          )}
        >
          <span className="text-sm font-medium text-muted-foreground">Notification Settings</span>
          <ChevronDown
            className={cn(
        ...
        <CollapsibleContent className="space-y-3 pt-2">
          <div className="space-y-2">
            <Label className="text-xs font-medium text-muted-foreground">Email Notifications</Label>
            <Select
          ...
          {emailNotification !== 'none' && (
            <motion.div
              ...
              className="space-y-2"
            >
              <Label htmlFor="notification-email-single" className="text-xs font-medium text-muted-foreground">
                Email Address
              </Label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input
                  id="notification-email-single"
                  type="email"
